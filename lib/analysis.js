...
/**
 * What one sensor is worth to the room score: the most it can ever take off, and
 * what it is taking off right now.
 *
 * Exported because the tiles explain themselves — a reader looking at 31 °C is
 * owed the answer to "and how much is that costing me". Derived from the same
 * SCORE_PENALTY table and the same shape functions `healthScore()` runs on, so
 * the explanation cannot drift from the arithmetic it describes.
 *
 * `null` for a sensor with no entry in the table: it is measured and shown, but
 * deliberately not scored (PM1 inside PM2.5, eCO2/TVOC off the SGP30's one VOC
 * signal — see config/sensors.js), and "0 points" would read as "it is fine"
 * rather than "it is not counted".
 *
 * `now` is computed from the raw latest value, not the rolling mean a PM tile
 * badges itself on: the score is what it is, and an explanation that quietly
 * used a different number would be explaining a different score.
 */
export function scoreWeight(sensor, reading) {
  const penalty = SCORE_PENALTY[sensor.id];
  if (!penalty) return null;

  const value = has(reading?.[sensor.field]) ? Number(reading[sensor.field]) : null;
  return {
    /** Points lost at the far end of the sensor's range — its full weight. */
    max: penalty.critical ?? penalty.danger,
    /** The rungs on the way there, for the reader who wants the ladder. */
    warn: penalty.warn,
    danger: penalty.danger,
    critical: penalty.critical ?? null,
    now: value == null ? null : PENALTY_BY_SHAPE[sensor.shape](value, THRESHOLDS[sensor.id], penalty),
  };
}
