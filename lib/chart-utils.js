...
/**
 * Project history rows into chronological chart arrays.
 * Values outside each sensor's physical range become gaps (null).
 *
 * Ordered by reading the timestamps, not by assuming them. This used to walk
 * the array backwards because the API answers newest-first — which drew the
 * whole chart right-to-left for any caller whose rows arrived in a different
 * order, with nothing on screen to say so. The x axis means time, so time is
 * what decides the order; a row with an unreadable timestamp has no place on a
 * time axis at all and is dropped rather than silently pinned to one end.
 *
 * The other two chart builders (bucketize in hooks/useFocus.js, and
 * buildPersonSeries in components/FocusSection.jsx) already sort. This was the
 * one that trusted its caller.
 */
