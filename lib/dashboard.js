...
export async function buildDashboard({ deviceId, hours, limit }) {
  ...
  const resolved =
    (deviceId && devices.includes(deviceId) && deviceId) || deviceId || devices[0] || null;
  ...
  const latestRow = history[0] ?? decorateRow(await getLatest(resolved));
  const latest = latestRow ? decorateRow(latestRow, { withAnalysis: true }) : null;
