...
export function mayReadFocus(provider) {
  // No provider named, no permission. Every caller passes one today, but both
  // tests below are written as "is this id on a list", and an `undefined` that
  // is on no list would have fallen through to the masked branch and been
  // granted — a gate whose default is "yes" is not a gate.
