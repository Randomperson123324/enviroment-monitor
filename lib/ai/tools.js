 ...
 *
 * ── Who may see what ─────────────────────────────────────────────────────────
 * Camera data (`focus`) is the one domain that must not reach a cloud provider.
 * That rule is enforced in three places on purpose, because one place is one
 * mistake away from a leak:
 *   1. `toolsFor(provider)` never offers the focus tool to a provider outside
 *      `config.ai.focusProviders` — the model cannot call what it cannot see.
 *   2. The system instruction tells that model it has no camera access, so it
 *      says so plainly instead of inventing numbers to fill the gap.
 ...
 * lib/ai/providers/*. `local_only` marks a tool no cloud provider may hold.
 */
export const TOOLS = [
  {
  ...
  {
    name: 'get_government_water',
    description:
      'ข้อมูลจากหน่วยงานรัฐ: ปริมาณฝน 24 ชม. สถานการณ์แม่น้ำ ระดับน้ำในเขื่อน/อ่างเก็บน้ำ และประกาศเตือนภัยน้ำ ' +
      '(กรมอุตุนิยมวิทยา · คลังข้อมูลน้ำแห่งชาติ · กรมชลประทาน)',
    parameters: { type: 'object', properties: {} },
    async run() {
      const gov = await getGovData();
      if (!gov) return { available: false, reason: 'ดึงข้อมูลหน่วยงานรัฐไม่สำเร็จ' };
      return {
        rainfall_top: (gov.rainfall ?? []).slice(0, config.gov.rainTopStations).map((r) => ({
          station: r.stationName ?? null,
          province: r.province?.th ?? null,
          rain_24h_mm: round(Number(r.rain24h)),
        })),
        river: gov.riverSituation
          ? {
              overflow_stations: gov.riverSituation.overflowCount ?? null,
              high_stations: gov.riverSituation.highCount ?? null,
              stations: (gov.riverSituation.stations ?? []).slice(0, 5),
            }
          : null,
        reservoirs: gov.reservoirs
          ? {
              total: gov.reservoirs.totalReservoirs ?? null,
              over_capacity: gov.reservoirs.overCapacityCount ?? null,
              high: gov.reservoirs.highCount ?? null,
            }
          : null,
        water_warnings: (gov.waterWarnings ?? []).slice(0, 5),
      };
    },
  },

  {
    name: 'get_flood_points',
    description:
      'สถานะจุดวัดระดับน้ำของโครงการ StreeFlood: จำนวนจุดที่อันตราย/เฝ้าระวัง/ปกติ และจุดที่น่ากังวลที่สุด ' +
      'พร้อมแนวโน้มระดับน้ำ (ซม./ชม.)',
    parameters: { type: 'object', properties: {} },
    async run() {
      const flood = await getFloodStatus();
      if (!flood?.summary) return { available: false, reason: 'ไม่มีข้อมูลจุดวัดน้ำ' };
      const worst = (flood.sensors ?? [])
        .filter((s) => s.level === 'danger' || s.level === 'warning')
        .slice(0, 5)
        .map((s) => ({
          name: s.name ?? null,
          level: s.level,
          water_cm: round(s.latest?.water_level_cm ?? null),
          trend_cm_per_hour: round(s.trend?.ratePerHour ?? null, 2),
        }));
      return { summary: flood.summary, of_concern: worst };
    },
  },
...

  {
    ...
    description:
      'ข้อมูลการจดจ่อจากกล้อง (pi-vision): การขยับ/หันหน้าต่อคน ทิศทาง และจำนวนคนที่ตรวจพบ ' +
      'เป็นข้อมูลที่ประมวลผลในเครื่องเท่านั้น',
    local_only: true,
    parameters: {
      type: 'object',
      properties: {
...

  {
    ...
    description:
      ...
      '"อากาศร้อนแล้วคนเสียสมาธิไหม" "ฝุ่นเยอะมีผลกับความตั้งใจไหม" ' +
      'คืนค่าสหสัมพันธ์ (r) พร้อมจำนวนช่วงเวลาที่ใช้คำนวณ และการเทียบ "ช่วงที่ค่าสูง vs ช่วงที่ค่าต่ำ" ตรง ๆ ' +
      '⚠️ ตัวชี้วัดฝั่งกล้องคือ "จำนวนครั้งที่หันหน้าออกจากท่านิ่งต่อคนต่อนาที" ซึ่ง**ยิ่งสูงยิ่งไม่จดจ่อ** ' +
...
export function mayReadFocus(provider) {
  return config.ai.focusProviders.includes(provider);
}
...
export function toolsFor(provider, { search = false } = {}) {
  return TOOLS.filter((t) => {
    if (t.local_only && !mayReadFocus(provider)) return false;
...
export async function runTool(name, args, ctx = {}) {
  ...
  if (tool.local_only && !mayReadFocus(ctx.provider)) {
    return {
