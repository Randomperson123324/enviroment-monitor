 ...
 *
 * ── This layer is what opens the camera data up ──────────────────────────────
 * `mayReadFocus()` in lib/ai/tools.js lets a cloud provider read the camera
 * aggregates *because* of what happens here, and only while it is switched on —
 * so a bug in this file is not a cosmetic one. Two rules follow from that:
 *   • every path that carries an identifier must mask it structurally (by field)
 *     rather than hope the roster catches it in prose — see `mask()` versus
 *     `maskText()`, and `learn()` for the payloads that are built as text;
 *   • an identifier that cannot be masked must not be sent, rather than sent and
 *     apologised for.
 * Raw frames are a separate matter and still never leave the Pi: what travels is
 * counts, directions and postures per variable — behaviour, not a face.
 */
...
function newSession() {
  ...
  return {
    tokenFor,
    /**
     * Register identifiers the caller knows are in the payload it is about to
     * send, before that payload is masked.
     *
     * The roster is a five-minute cache of who exists; a summary built from the
     * newest camera rows can legitimately contain a name that appeared after it
     * was filled. Text masking can only replace what it has been told to look
     * for, so the builder — which is holding the actual rows — says so, and the
     * gap closes at the one place that can see it. See lib/ai/summaries.js.
     */
    learn(entries) {
      for (const e of entries ?? []) {
        if (e?.value != null) tokenFor(e.kind ?? 'NAME', e.value);
      }
    },
    mask,
    maskText,
