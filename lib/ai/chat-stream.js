...
import { mayReadFocus, runTool, toolsFor } from '@/lib/ai/tools';

/**
 * What the model is told about itself.
 *
 * The cloud case is the point of the second paragraph: a model with no camera
 * tool, asked about focus, will otherwise reach for the nearest plausible thing
 * — the sensor data — and answer as though it had looked. Naming the limit turns
 * a confident wrong answer into a correct refusal, and gives the user something
 * actionable ("ask the on-device model") instead of a shrug.
 */
...
async function* streamOne(id, { message, history, deviceId, lang, search, thinking, overrides, deadline }) {
  ...
  const transcript = initialTurns(id, history, message);
  ...
  for (let round = 0; round < config.ai.tools.maxRounds; round++) {
    ...
    const common = {
      ...
      apiKey: s.apiKey,
      model,
      systemInstruction: systemPrompt(id, { search, hasTools: tools.length > 0 }),
      tools,
      thinking,
      thinkingBudget: config.ai.stream.thinkingBudget,
    ...
    for (const c of pending) yield { type: 'tool-start', name: c.name, args: c.args };
    ...
    const results = await Promise.all(
      pending.map(async (c) => {
        const result = c.refused
          ...
          : await runTool(c.name, c.args, { provider: id, deviceId, lang });
        return { id: c.id, name: c.name, args: c.args, result };
      })
    );

    for (const r of results) {
      yield { type: 'tool', name: r.name, args: r.args, ok: !r.result?.error, note: r.result?.error ?? null };
    }

    appendToolRound(id, transcript, pending, results);
