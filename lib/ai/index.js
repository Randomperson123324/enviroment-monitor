...
async function callProvider(
  id,
  { systemInstruction, messages, jsonOutput, identifiers },
  overrides,
  { budgetMs, forceDiscover, model: explicitModel } = {}
) {
  const s = providerSettings(id, overrides);
  const model =
    explicitModel ?? (await resolveModel(id, overrides, { forceDiscover, timeoutMs: budgetMs }));

  /**
   * Identifiers are swapped for variables here rather than in each caller,
   * because here is the last place a payload is still one object — every path
   * to a provider passes through this function, so none of them can forget.
   * `alias` is null for a provider trusted with real names, and the callers
   * hand it back what they got: text through `unmask`, parsed JSON through
   * `unmaskDeep`.
   */
  const alias = await createAliasSession(id);
  // Whoever built this payload may know identities the roster does not yet — a
  // name recognised since it was cached. Registered before the masking runs, so
  // they are swapped rather than merely mapped.
  if (alias) alias.learn(identifiers);
  const outbound = alias ? messages.map((m) => ({ ...m, content: alias.maskText(m.content) })) : messages;

  const common = {
    baseUrl: s.baseUrl,
    apiKey: s.apiKey,
    model,
    // Masked first, then the note is prepended — the note's own {{NAME_1}}
    // examples are the format being explained, not data to be swapped.
    systemInstruction: withAliasNote(
      alias,
      alias ? alias.maskText(systemInstruction) : systemInstruction
    ),
    jsonOutput,
  ...
  const text =
    id === 'gemini'
      ? await PROVIDERS.gemini.impl.generate({
          ...common,
          contents: outbound.map((m) => ({
            role: m.role === 'assistant' ? 'model' : 'user',
            parts: [{ text: String(m.content ?? '') }],
          })),
        })
      : await PROVIDERS[id].impl.generate({ ...common, messages: outbound, thinking: s.thinking });

  return { text, provider: id, model, alias };
}
...
async function runChain(request, overrides, only) {
  ...
  for (const id of order) {
    ...
    try {
      ...
      const pinnedModel = providerSettings(id, overrides).model;
      if (pinnedModel && isModelRejected(err.message) && left() > config.ai.minAttemptMs) {
        const candidates = await modelCandidates(id, overrides, {
          exclude: pinnedModel,
          timeoutMs: left(),
        }).catch(() => []);
        ...
        const tried = [];
        let lastErr = err;
        for (const model of candidates.slice(0, config.ai.modelRetryMax)) {
          if (left() < config.ai.minAttemptMs) break;
          try {
            const res = await callProvider(id, request, overrides, { budgetMs: left(), model });
            console.warn(`[ai] ${id} recovered on ${model} after ${pinnedModel} was refused`);
            return res;
          } catch (retryErr) {
            tried.push(model);
            lastErr = retryErr;
        ...
        errors.push(
          `${id}: ${err.message}${tried.length ? ` (also refused ${tried.join(', ')}: ${lastErr.message})` : ''}`
        );
        continue;
      }
...
export async function aiAnalyze({ reading, summaryLine, overrides = {} }) {
  ...
  const { text, provider, model, alias } = await runChain(
    { messages: [{ role: 'user', content: prompt }], jsonOutput: true },
    overrides
  );

  /**
   * Names come back after the JSON is parsed, never before: a restored value is
   * arbitrary text, and one containing a quote would turn valid JSON into a
   * parse error the moment it was pasted back into the string.
   */
  const parsed = alias ? alias.unmaskDeep(parseJsonLoose(text)) : parseJsonLoose(text);
 ...
 */
export async function aiSummarize({ instruction, contextText, identifiers, overrides = {}, only }) {
  const maxRecs = config.ai.summary.maxRecs;
  const prompt = `${instruction}\n\n${fill(MSG.scopes.jsonShape, { maxRecs })}\n\n${contextText}`;
  const { text, provider, model, alias } = await runChain(
    { messages: [{ role: 'user', content: prompt }], jsonOutput: true, identifiers },
    overrides,
    only
  );

  // Parse first, restore after — see aiAnalyze.
  const parsed = alias ? alias.unmaskDeep(parseJsonLoose(text)) : parseJsonLoose(text);
  const recommendations = Array.isArray(parsed.recommendations)
    ? parsed.recommendations
        .filter((r) => r && typeof r.text === 'string')
  ...
  return {
    ...
    model,
    /**
     * True when this context held identities and the provider was shown
     * variables instead. Only the scopes that carry names declare any, so the
     * card can say so where it is true rather than everywhere it is possible.
     */
    masked: Boolean(alias && identifiers?.length),
  };
}
...
export async function aiChat({ message, history = [], contextLine, overrides = {} }) {
  ...
  return { reply: alias ? alias.unmask(text) : text, provider, model };
}
