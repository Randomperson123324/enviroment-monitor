 ...
 */
/**
 * Does this failure mean "the model we asked for is gone", as opposed to "the
 * provider is down"? Google answers 404 for a name it has retired for new keys —
 * including names its own list endpoint still returns — so the recovery is to ask
 * what the key can call and try that once.
 */
...
async function runChain(request, overrides, only) {
  ...
  for (const id of order) {
    ...
    try {
      ...
      if (isModelRejected(err.message) && left() > config.ai.minAttemptMs) {
        try {
