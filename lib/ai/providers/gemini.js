...
export async function* streamGenerate({
  ...
  if (!res.ok) {
    ...
    /**
     * Retry without thoughts only when thinking is plausibly the cause: either
     * Google names it, or it names nothing at all (the flat "Request contains an
     * invalid argument.", which is what an unsupported generationConfig field
     * gets). A 400 that names something specific — a missing thought_signature,
     * a bad tool schema — is reported instead.
     *
     * Retrying on *any* 400 was worse than not retrying at all: it turned the
     * missing-signature error into a silent thoughts-free answer, so the bug
     * presented as "the thinking button does nothing".
     */
    const blamesThinking = /thinking/i.test(detail);
    const blamesNothing = /Request contains an invalid argument/i.test(detail);
    if (res.status === 400 && thinkingConfig && (blamesThinking || blamesNothing)) {
      console.warn('[gemini] thinking rejected, retrying without thoughts:', detail.slice(0, 140));
