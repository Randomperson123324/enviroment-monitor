...
import { F32_FALLBACKS, f32SiblingId } from '@/lib/ai/browser/models';
...
export async function resolveModelId(modelId) {
  ...
  const curated = F32_FALLBACKS[modelId];
  if (curated) return curated.id;

  // A model picked from the Hugging Face search has no curated twin, but MLC
  // names the two builds alike: swap the quantisation and take it only if this
  // web-llm actually ships a compiled library for it.
