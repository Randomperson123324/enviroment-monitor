...
def draw_phones(img, boxes):
    ...
    ใต้นโยบายปกติ (`PHONE_NEAREST_ALWAYS`) ทุกกรอบที่วาดที่นี่จะมีเจ้าของเสมอ กรอบนี้
    จึงบอกว่า**ตัวตรวจเห็นอะไรบ้าง** ซึ่งเป็นคนละคำถามกับ "ใครถูกนับว่าใช้อยู่"
    — เห็นกรอบขึ้นรอบรีโมทหรือกระเป๋าดินสอเมื่อไร นั่นคือสัญญาณว่าควรเพิ่ม
    PHONE_MIN_SCORE ก่อนที่ของชิ้นนั้นจะถูกยกให้ใครสักคนทุกเฟรม

    และเมื่อป้าย `PHONE` ไม่ขึ้นทั้งที่คนกำลังเล่นอยู่ชัด ๆ กรอบนี้แยกสาเหตุให้ทันที:
    ไม่มีกรอบเลย = ตัวตรวจไม่เห็นเครื่องนั้น (ลด PHONE_MIN_SCORE) · มีกรอบแต่ป้ายไป
    ขึ้นที่คนอื่น = จับคู่ผิดคน (ดูว่าใครอยู่ใกล้กว่ากันจริง ๆ ในเฟรม)
    """
    h, w = img.shape[:2]
