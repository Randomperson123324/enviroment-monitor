...
CONFIG = {
    ...
    # ── ใช้โทรศัพท์อยู่หรือเปล่า (MediaPipe Object Detector) ──
    # ใช้เฉพาะโหมดติดตามกิจกรรมเหมือน body — และ**ต้องมี body เปิดอยู่ด้วย** เพราะ
    # การบอกว่า "ใครถือ" ต้องรู้ตำแหน่งข้อมือ ซึ่งมาจากโมเดลท่าทางเท่านั้น
    # ⚠️ ตัวตรวจอย่างเดียวตอบได้แค่ "มีโทรศัพท์อยู่ในเฟรม" ซึ่งเป็นคนละคำถาม — ดูหัวไฟล์ phone.py
    "phone": {
        "enabled": _bool("PHONE_ENABLED", True),
        "path": _str("PHONE_MODEL_PATH", str(BASE_DIR / "models" / "efficientdet_lite0.tflite")),
        # โมเดล COCO 90 คลาส · lite0 (13 MB, input 320px) เร็วพอสำหรับ Pi
        # เปลี่ยนเป็น efficientdet_lite2 (input 448px) ถ้าต้องการเห็นโทรศัพท์ที่อยู่ไกลขึ้น
        # และยอมจ่าย CPU เพิ่ม — ทั้งสองตัวใช้ label เดียวกัน จึงสลับได้ด้วยการแก้สองบรรทัดนี้
        "url": _str(
            "PHONE_MODEL_URL",
            "https://storage.googleapis.com/mediapipe-models/object_detector/"
            "efficientdet_lite0/float32/1/efficientdet_lite0.tflite",
        ),
        "auto_download": _bool("PHONE_AUTO_DOWNLOAD", True),
        # ⚠️ ตัวคุมราคาหลัก · **ถูกปัดขึ้นให้เป็นจำนวนเท่าของ BODY_EVERY_N_FRAMES เสมอ**
        # เพราะการจับคู่ต้องใช้ข้อมือของเฟรมเดียวกัน ไม่ใช่ข้อมือของเมื่อสี่เฟรมก่อน
        # 8 = ราว 2–3 ครั้ง/วินาทีที่ 20fps ซึ่งเร็วกว่าการหยิบโทรศัพท์ขึ้นมาดูมาก
        "every_n_frames": _int("PHONE_EVERY_N_FRAMES", 8),
        # คะแนนขั้นต่ำที่ยอมรับว่าเป็นโทรศัพท์ — ตั้งสูงกว่าตัวตรวจใบหน้าเพราะของ
        # สี่เหลี่ยมมืด ๆ ในห้อง (รีโมท กระเป๋าดินสอ ขอบโน้ตบุ๊ก) หน้าตาเหมือนโทรศัพท์มาก
        "min_score": _num("PHONE_MIN_SCORE", 0.4),
        # กี่กรอบต่อเฟรม — ห้องหนึ่งไม่ได้มีโทรศัพท์เปิดใช้พร้อมกันเกินนี้ในทางปฏิบัติ
        "max_results": _int("PHONE_MAX_RESULTS", 10),
        # ── โทรศัพท์ทุกเครื่องต้องมีเจ้าของไหม ──
        # true  = ยกให้คนที่อยู่ใกล้ที่สุดเสมอ แม้ไม่มีหลักฐานว่าถืออยู่ (ค่าเริ่มต้น)
        # false = ไม่มีหลักฐาน (ข้อมือ/ช่องใต้หน้า) ก็ไม่นับให้ใคร — โหมดเข้ม
        # ⚠️ true แปลว่าโทรศัพท์ที่วางบนโต๊ะก็ถูกนับว่ามีคนใช้ด้วย · ธง phone_confident
        # ยังแยก "เห็นอยู่ในมือจริง" ออกจาก "แค่ใกล้ที่สุด" ให้ — ดูหัวไฟล์ phone.py
        "nearest_always": _bool("PHONE_NEAREST_ALWAYS", True),
        # ── ระยะที่ถือว่า "อยู่ในมือคนนี้" (ชั้นที่ให้ confident=true) ──
        # เป็นสัดส่วนของ**ความกว้างไหล่** (เหมือนทุกเกณฑ์ใน body.py) ไม่ใช่พิกเซล
        # 0.6 ≈ ระยะที่มือเอื้อมถึงจริง เผื่อ landmark ข้อมือคลาดไปบ้าง
        "hand_reach": _num("PHONE_HAND_REACH", 0.6),
        # ── "ช่องใต้ใบหน้า" — ท่าที่คนถือโทรศัพท์อยู่จริง ──
        # ไม่ได้ใช้คัดคนออกจากการจับคู่แล้ว (นโยบายใกล้ที่สุดยกให้ทุกเครื่องอยู่ดี)
        # แต่ใช้**จัดลำดับ**: คนที่โทรศัพท์อยู่ใต้หน้าพอดี ชนะคนที่บังเอิญนั่งใกล้กว่า
        # เป็นสัดส่วนของ**ความกว้างใบหน้า** · วัดสองแกนแยกกันเพราะมือของคนอยู่ในแนวดิ่ง
        # ใต้หัว ไม่ใช่เฉียงออกข้าง — เหตุผลเต็มอยู่ที่ `_in_column` ใน phone.py
        "face_side": _num("PHONE_FACE_SIDE", 0.75),
        "face_below": _num("PHONE_FACE_BELOW", 2.5),
        # ── กันป้ายกระพริบ (phone.PhoneHold) ──
        "hold_seconds": _num("PHONE_HOLD_SECONDS", 0.6),
        # ยาวกว่า hold เพราะตัวตรวจวัตถุหลุดเป็นเฟรม ๆ ตอนมือขยับหรือแสงสะท้อน
        "release_seconds": _num("PHONE_RELEASE_SECONDS", 2.0),
    },
